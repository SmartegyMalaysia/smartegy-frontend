...
const paymentStatuses: Array<PaymentStatus | "all"> = ["all", "not_recorded", "pending_verification", "current", "overdue", "partially_paid", "fully_paid"];
const sortOptions = ["customer", "agent", "amount", "updated"] as const;
type SortKey = (typeof sortOptions)[number];
const sortLabels: Record<SortKey, string> = { customer: "customer", agent: "agent", amount: "amount", updated: "updated date" };
const pageSize = 5;
...
export function CaseQueue({ actor, isAgent, showCount = false, title, description }: { actor: CurrentUser; isAgent: boolean; showCount?: boolean; title?: string; description?: string }) {
  ...
  const load = useCallback(async () => {
    ...
    setRefreshing(true);
    const result = await casesRepository.listPage(actor, { search: debouncedSearch, stage: stage === "all" ? undefined : stage, paymentStatus: paymentStatus === "all" ? undefined : paymentStatus, agentId: agentFilter === "all" ? undefined : agentFilter, page, pageSize, sortBy: sort, sortDirection });
    if (currentRequest !== requestId.current) return;
    if (result.ok) { hasLoaded.current = true; setData(result.data); setState("ready"); } else if (!hasLoaded.current) setState("error");
    setRefreshing(false);
  }, [actor, agentFilter, debouncedSearch, page, paymentStatus, sort, sortDirection, stage]);
  useEffect(() => { if (search === debouncedSearch) void load(); }, [debouncedSearch, load, search]);
  ...
  async function exportCases() { await casesRepository.export(actor, { search: debouncedSearch, stage: stage === "all" ? undefined : stage, paymentStatus: paymentStatus === "all" ? undefined : paymentStatus, agentId: agentFilter === "all" ? undefined : agentFilter, sortBy: sort, sortDirection }); }
  ...
  function sortableHeader(key: SortKey, heading = sortLabels[key]): DataTableHeader { const direction = sort === key ? sortDirection : undefined; return { ariaSort: direction === "asc" ? "ascending" : direction === "desc" ? "descending" : undefined, content: <button className="table-sort-button" type="button" onClick={() => updateSort(key)} aria-label={`Sort by ${sortLabels[key]}${direction ? `, currently ${direction === "asc" ? "ascending" : "descending"}` : ""}`}><span>{heading}</span><span className="table-sort-indicator" aria-hidden="true">{direction === "asc" ? "↑" : direction === "desc" ? "↓" : "↕"}</span></button> }; }
  ...
  return <section className="panel recent-panel case-table-panel">
    ...
    <div className="case-filters" aria-label="Case filters">
      ...
      <label><span>Payment Status</span><FilterSelect allLabel="All payment statuses" labels={{ all: "All payment statuses", not_recorded: "Not recorded", pending_verification: "Pending verification", current: "Current", overdue: "Overdue", partially_paid: "Partially paid", fully_paid: "Fully paid" }} value={paymentStatus} options={paymentStatuses} onChange={(value) => { setPaymentStatus(value as PaymentStatus | "all"); setPage(1); }} /></label>
    ...
    {visibleCases.length ? <div className="desktop-case-table"><DataTable caption="All cases" headers={isAgent ? ["Case", sortableHeader("customer", "Customer"), sortableHeader("amount", "Amount"), "Status", "Payment Status", sortableHeader("updated", "Updated")] : ["Case", sortableHeader("customer", "Customer"), sortableHeader("agent", "Agent"), sortableHeader("amount", "Amount"), "Status", "Payment Status", sortableHeader("updated", "Updated")]}>{visibleCases.map((item) => <tr className="case-table-row" key={item.id} tabIndex={0} role="link" aria-label={`Open case ${item.caseNumber}`} onClick={() => openCase(item.id)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); openCase(item.id); } }}><td><Link className="table-primary" href={`/cases/${item.id}`}>{item.caseNumber}</Link></td><td>{item.customerDisplayName}{isAgent && <span className="table-secondary">{item.agentName}</span>}</td>{!isAgent && <td>{item.agentName}</td>}<td className="commission-money">{formatMoney(item.saleAmountSen)}</td><td><Badge status={item.status} /></td><td><Badge status={item.paymentStatus} /></td><td className="muted-cell">{formatDate(item.updatedAt)}</td></tr>)}</DataTable></div> : <EmptyState title="No matching cases" description="Try changing or clearing the filters." />}
