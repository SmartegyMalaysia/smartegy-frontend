...
export function RegistrationQueuePage() {
  ...
  return <AppShell user={user} onRoleChange={setRole}><div className="page-content"><div className="page-header"><div><p className="eyebrow">Staff operations</p><h1>Agent Registration</h1><p className="page-description">Find applications awaiting profile and payment review.</p></div></div><div className="preview-banner"><span className="preview-dot"/><div><strong>Manual verification</strong><span> Payment proof is reviewed by authorised staff. No payment is verified automatically.</span></div></div>{role === "agent" ? <PermissionDenied/> : loading ? <LoadingState/> : failed ? <ErrorState onRetry={load}/> : <section className="panel recent-panel case-table-panel"><div className="panel-header case-table-header"><div><h2>Agent registrations</h2><p>Select an application to review its payment proof and audit history.</p></div><span className="case-count">{registrations.length} applications</span></div><QueueFilters query={query} update={update} reset={reset} hasFilters={hasFilters}/>{registrations.length === 0 ? <EmptyState title={query.search || hasFilters ? "No matching applications" : "No registrations yet"} description={query.search || hasFilters ? "Try changing or clearing the filters." : "New applications will appear after applicants submit their registration."}/> : <><div className="desktop-case-table"><DataTable caption="Agent registration queue" headers={["User ID", "Name", "Phone number", "Email", "Upline agent", "Payment verified", "Profile"]}>{visible.map((registration) => <RegistrationRow key={registration.id} registration={registration}/>)}</DataTable></div><div className="case-table-footer"><span className="case-page-summary">Showing {visible.length ? (currentPage - 1) * pageSize + 1 : 0}&ndash;{Math.min(currentPage * pageSize, registrations.length)} of {registrations.length}</span><div className="pagination" aria-label="Registration queue pagination"><button className="pagination-button" type="button" aria-label="Previous page" disabled={currentPage === 1} onClick={() => setPage((value) => value - 1)}>&lsaquo;</button><span>Page {currentPage} of {totalPages}</span><button className="pagination-button" type="button" aria-label="Next page" disabled={currentPage === totalPages} onClick={() => setPage((value) => value + 1)}>&rsaquo;</button></div></div></>}</section>}</div></AppShell>;
}
