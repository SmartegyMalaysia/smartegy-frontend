...
export function RegistrationSignup({ referralCode }: { referralCode: string }) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (stage === "payment") {
      ...
      setUploadError(null);
      setSubmitting(true);
      const submitted = await registrationRepository.submitFee(actorFor(registration.id), { registrationId: registration.id, paymentDate: null, paymentReference: null, paymentRemarks: String(form.get("paymentRemarks") ?? ""), proof: { fileName: file?.name ?? "", mimeType: file?.type ?? "", sizeBytes: file?.size ?? 0, file: file ?? undefined } });
...
function RegistrationFields({ fieldErrors }: { fieldErrors: Record<string, string[]> }) {
  return <><div className="form-section-heading"><h2>Initial information</h2><p>Complete these six details to receive an email OTP.</p></div><div className="registration-form-grid"><Field id="fullName" label="Full name" error={fieldErrors.fullName}><input id="fullName" name="fullName" type="text" autoComplete="name" required /></Field><Field id="email" label="Email address" error={fieldErrors.email}><input id="email" name="email" type="email" autoComplete="email" required /></Field><Field id="mobileNumber" label="Mobile number" error={fieldErrors.mobileNumber}><input id="mobileNumber" name="mobileNumber" type="tel" autoComplete="tel" placeholder="e.g. 012345678" required /></Field><Field id="referralCode" label="Invitation / referral code"><input id="referralCode" value="Confirmed from invitation link" readOnly /></Field><Field id="password" label="Password" error={fieldErrors.password}><input id="password" name="password" type="password" autoComplete="new-password" minLength={8} required /></Field><Field id="passwordConfirmation" label="Confirm password" error={fieldErrors.passwordConfirmation}><input id="passwordConfirmation" name="passwordConfirmation" type="password" autoComplete="new-password" minLength={8} required /></Field></div><p className="field-help">Your confirmed upline cannot be changed by the applicant.</p><label className={`terms-row ${fieldErrors.acceptedTerms ? "terms-row-error" : ""}`}><input name="acceptedTerms" type="checkbox" required aria-describedby={fieldErrors.acceptedTerms ? "acceptedTerms-error" : undefined} /><span>I accept the <a href="#terms">Terms of Use</a> and <a href="#privacy">Privacy Notice</a>.</span></label>{fieldErrors.acceptedTerms && <p id="acceptedTerms-error" className="field-error" role="alert">{fieldErrors.acceptedTerms[0]}</p>}</>;
}
...
function OtpStep({ email, fieldErrors, resendCooldown, onResend }: { email: string; fieldErrors: Record<string, string[]>; resendCooldown: number; onResend: () => void }) {
  return <div className="otp-step"><div className="form-section-heading"><h2>Check your email</h2><p>Enter the 6-digit verification code sent to {email}. The code expires in 10 minutes.</p></div><Field id="otp" label="Email OTP" error={fieldErrors.otp}><input id="otp" name="otp" inputMode="numeric" autoComplete="one-time-code" maxLength={6} placeholder="Enter your 6-digit code" required /></Field><div className="otp-actions"><span>Didn’t receive it?</span><button className="text-button" type="button" disabled={resendCooldown > 0} onClick={onResend}>{resendCooldown ? `Resend in ${resendCooldown}s` : "Resend OTP"}</button></div></div>;
}

function PaymentStep({ registration, paymentConfig, uploadError, onUploadChange }: { registration: AgentRegistration; paymentConfig: RegistrationPaymentConfig; uploadError: string | null; onUploadChange: (error: string | null) => void }) {
  return <div className="payment-step"><div className="form-section-heading"><h2>Registration fee: {formatMoney(paymentConfig.feeAmountSen)}</h2><p>Pay the non-refundable RM50 name-card fee, then upload your proof of payment.</p></div><div className="registration-invoice"><div><p className="detail-label">Invoice</p><strong>{registration.invoice.invoiceNumber}</strong><span>{registration.invoice.description}</span></div><div><p className="detail-label">Amount due</p><strong>{formatMoney(registration.invoice.amountSen)}</strong><span>Issued {registration.invoice.issueDate}</span></div></div><div className="payment-account-grid"><div><span className="detail-label">Bank name</span><strong>{paymentConfig.bankName}</strong></div><div><span className="detail-label">Account name</span><strong>{paymentConfig.accountName}</strong></div><div><span className="detail-label">Account number</span><strong>{paymentConfig.accountNumber}</strong></div><div><span className="detail-label">Transfer reference</span><strong>{registration.applicationNumber}</strong><small>Use this application number when making the transfer.</small></div></div><div className="payment-qr-card"><strong>DuitNow QR</strong><span>{paymentConfig.duitNowQrAvailable ? "Scan the QR supplied by Smartegy." : "QR will be provided by Smartegy later."}</span></div><div className="registration-field"><label htmlFor="proof">Proof of payment</label><PaymentProofUpload error={uploadError ?? undefined} onFileChange={(file) => onUploadChange(file ? null : uploadError)} />{!uploadError && <p className="field-help payment-upload-help">Upload the transfer confirmation for manual staff verification.</p>}</div><div className="registration-field"><label htmlFor="paymentRemarks">Remarks <span className="field-help-inline">(optional)</span></label><textarea id="paymentRemarks" name="paymentRemarks" rows={3} placeholder="Add any context for staff, if needed" /></div></div>;
}
