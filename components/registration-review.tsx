...
export function RegistrationReviewPage({ applicationNumber }: { applicationNumber: string }) {
  ...
  async function run(resultPromise: Promise<RegistrationActionResult<AgentRegistration>>, success: string) { setActionLoading(true); try { const result = await resultPromise; if (result.ok) { setRegistration(result.data); setToast({ title: "Action completed", subtitle: success, tone: "success" }); setAction(null); setPaymentReason(""); } else setToast({ title: "Action failed", subtitle: result.error.message, tone: "error" }); } finally { setActionLoading(false); } }
...
function PaymentReview({ actor, registration, onProof, onVerify, onReject }: { actor: Parameters<typeof registrationRepository.getPaymentProof>[0]; registration: AgentRegistration; onProof: (proof: RegistrationPaymentProofAccess) => void; onVerify: (details: VerifyRegistrationFeeInput) => void; onReject: () => void }) {
  ...
  useEffect(() => { setAmount(((registration.verifiedAmountSen ?? registration.feeAmountSen) / 100).toFixed(2)); setDate(registration.verifiedPaymentDate ?? registration.paymentDate ?? new Date().toISOString().slice(0, 10)); setBankReference(registration.bankReference ?? registration.paymentReference ?? registration.applicationNumber); setFieldErrors({}); }, [registration.id, registration.verifiedAmountSen, registration.feeAmountSen, registration.verifiedPaymentDate, registration.paymentDate, registration.bankReference, registration.paymentReference, registration.applicationNumber]);
  function verify() {
