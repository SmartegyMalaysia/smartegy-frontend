...
export function CaseWorkspace({ initialCase, user, onChanged, onDeleted }: { initialCase: CaseDetail; user: CurrentUser; onChanged: (value: CaseDetail) => void; onDeleted: () => void }) {
  ...
  return <div className="case-workspace">
    ...
    <ConfirmationDialog open={recordPaymentOpen} title="Record Deposit" description="Enter the deposit amount and date. The deposit will be allocated automatically and confirmed immediately." confirmLabel="Confirm Deposit" confirmVariant="primary" confirmDisabled={busy || Boolean(paymentError)} onCancel={closeRecordPayment} onConfirm={recordPayment}>
