...
const savingsMonthNumberOptions = Array.from({ length: 12 }, (_, index) => String(index + 1).padStart(2, "0"));
const savingsMonthNumberLabels = Object.fromEntries(savingsMonthNumberOptions.map((month) => [month, new Intl.DateTimeFormat("en-MY", { month: "long", timeZone: "Asia/Kuala_Lumpur" }).format(new Date(`2000-${month}-01T00:00:00`))]));
...
type SavingsReadingDraft = { month: string; monthNumber: string; year: string; bill: string; kwh: string };
const emptySavingsReadings = (): SavingsReadingDraft[] => Array.from({ length: 3 }, () => ({ month: "", monthNumber: "", year: "", bill: "", kwh: "" }));
function monthWithOffset(year: string, monthNumber: string, offset: number) {
  const date = new Date(Date.UTC(Number(year), Number(monthNumber) - 1 + offset, 1));
  return `${date.getUTCFullYear()}-${String(date.getUTCMonth() + 1).padStart(2, "0")}`;
}
function updateSavingsReading(readings: SavingsReadingDraft[], index: number, patch: Partial<Pick<SavingsReadingDraft, "monthNumber" | "year" | "bill" | "kwh">>) {
  const next = readings.map((reading, readingIndex) => readingIndex === index ? { ...reading, ...patch } : reading);
  const changed = next[index];
  if (patch.monthNumber !== undefined || patch.year !== undefined) {
    next[index] = { ...changed, month: changed.year && changed.monthNumber ? `${changed.year}-${changed.monthNumber}` : "" };
  }
  const anchor = next[index];
  if (anchor.monthNumber && anchor.year.length === 4) {
    for (let rowIndex = 0; rowIndex < next.length; rowIndex += 1) {
      if (rowIndex === index) continue;
      const reading = next[rowIndex];
      if (!reading.month && !reading.monthNumber && !reading.year) {
        const month = monthWithOffset(anchor.year, anchor.monthNumber, rowIndex - index);
        next[rowIndex] = { ...reading, month, monthNumber: month.slice(5), year: month.slice(0, 4) };
      }
    }
  }
  return next;
...
function savingsReadingDrafts(readings: ProposalEnergyReading[] | undefined): SavingsReadingDraft[] {
  ...
  return readings.slice(0, 3).map((reading) => ({ month: reading.month, monthNumber: reading.month.slice(5, 7), year: reading.month.slice(0, 4), bill: String(reading.billAmountSen / 100), kwh: String(reading.kwhUsed) })).concat(emptySavingsReadings()).slice(0, 3);
}
...
function PaymentRecordsPanel({ payments, schedules, documents, role, selectedPaymentId, embedded = false, onViewProof, onVerifyPayment }: { payments: CasePayment[]; schedules: PaymentSchedule[]; documents: CaseDocument[]; role: CurrentUser["role"]; selectedPaymentId?: string | null; embedded?: boolean; onViewProof: (payment: CasePayment) => void; onVerifyPayment: (paymentId: string) => void }) {
  ...
  const content = <>
    <div className="case-payment-record-list">{records.length ? records.map((payment) => {
      ...
      const selectable = payment.status === "pending_verification" && role !== "agent";
      const selectPayment = () => { if (selectable) onVerifyPayment(payment.id); };
      return <article className={`case-payment-record ${selectable ? "case-payment-record-selectable" : ""} ${selectedPaymentId === payment.id ? "case-payment-record-selected" : ""}`} key={payment.id} role={selectable ? "button" : undefined} tabIndex={selectable ? 0 : undefined} aria-pressed={selectable ? selectedPaymentId === payment.id : undefined} onClick={selectPayment} onKeyDown={(event) => { if (selectable && (event.key === "Enter" || event.key === " ")) { event.preventDefault(); selectPayment(); } }}>
        <div className="case-payment-record-main"><div className="case-payment-record-heading"><strong>{formatMoney(payment.amountSen)}</strong><Badge status={payment.status} /></div><div className="case-payment-record-meta"><span>Paid on {formatDate(payment.paymentDate)}</span><span>Submitted {formatDateTime(payment.recordedAt)}</span>{payment.reference && <span>Reference: {payment.reference}</span>}</div>{payment.rejectionReason && <p className="case-payment-record-reason">Rejection reason: {payment.rejectionReason}</p>}{proofName && <button className="case-payment-proof-link" type="button" onClick={(event) => { event.stopPropagation(); onViewProof(payment); }}>View proof: {proofName}</button>}</div>
        <div className="case-payment-record-actions">{payment.status === "pending_verification" ? role === "agent" ? <span className="case-payment-pending-note">Pending staff verification</span> : <Button type="button" variant="secondary" size="sm" onClick={(event) => { event.stopPropagation(); onVerifyPayment(payment.id); }}>Select</Button> : payment.status === "verified" ? <span className="case-payment-pending-note">Assigned to schedule</span> : null}</div>
      </article>;
...
function FilePreviewActions({ access, showDownload = true, showOpenLink = true, openLabel = "Open in new tab" }: { access: PaymentProofAccess; showDownload?: boolean; showOpenLink?: boolean; openLabel?: string }) {
  const hasUrl = Boolean(access.url.trim());
  ...
  return <>{showDownload && <Button type="button" variant="secondary" onClick={downloadFile} disabled={!hasUrl}><Icon name="download" size={15} />Download</Button>}{showOpenLink && <a className={`button button-secondary${!hasUrl ? " button-disabled" : ""}`} href={hasUrl ? access.url : undefined} target="_blank" rel="noreferrer" aria-disabled={!hasUrl}><Icon name="external-link" size={15} />{openLabel}</a>}</>;
}
...
export function CaseWorkspace({ initialCase, user, onChanged, onDeleted }: { initialCase: CaseDetail; user: CurrentUser; onChanged: (value: CaseDetail) => void; onDeleted: () => void }) {
  ...
  const [documentPreviewAccess, setDocumentPreviewAccess] = useState<PaymentProofAccess | null>(null);
  ...
  function toggleDetailsEditing() {
    setDetailsError(null);
    if (editing) {
      setEditing(false);
      return;
    }
    if (caseDetail.proposal?.status === "accepted") {
  ...
  function openDialog(action: CaseActionKind) {
    ...
    if (action === "verify_savings") { setSavingsError(null); setSavingsReadings(emptySavingsReadings()); }
    if (action === "resubmit") setResubmitError(null);
    setDialog(action);
  }
  function closeDialog() { setDialog(null); setRecordPaymentOpen(false); setReason(""); setQuoteError(null); setPaymentError(null); setAllocationError(null); setVerificationPaymentId(null); setSavingsError(null); setSavingsReadings(emptySavingsReadings()); setResubmitError(null); }
  ...
  function openDocumentInNewTab(access: PaymentProofAccess) {
    const opened = window.open(access.url, "_blank", "noopener,noreferrer");
    if (!opened) showError("Allow pop-ups to open the document in a new tab.", "Document Access Failed");
  }
  ...
  async function previewTimelineDocument(document: { id: string; name: string; documentIds: string[]; regenerateType: "quotation" | "proforma" | null; sourceId?: string }) {
    ...
    const source = caseDetail.documents.find((item) => item.id === documentId);
    setDocumentPreviewAccess({ fileName: source?.fileName ?? `${document.name}.docx`, mimeType: source?.mimeType ?? "application/vnd.openxmlformats-officedocument.wordprocessingml.document", url: result.data });
  }
  async function generateDocument(type: "proforma", paymentScheduleId?: string) {
    ...
    if (result.ok) {
      ...
      if (result.data.signedUrl) {
        const opened = window.open(result.data.signedUrl, "_blank", "noopener,noreferrer");
        if (!opened) showError("Allow pop-ups to open the invoice in a new tab.", "Invoice Generated");
      }
      setToast({ title: "Invoice Generated", subtitle: `${result.data.documentNumber} is available in Documents.`, tone: "success" });
    } else showError(result.error.message, "Document Generation Failed");
    setBusy(false);
  }
  const documentRows = [
    ...caseDetail.documents.filter((document) => document.type !== "signed_proposal" || document.id === caseDetail.proposal?.signedDocumentId).map((document) => ({ kind: "uploaded" as const, id: document.id, date: document.uploadedAt, label: uploadedDocumentLabel(document.type), tag: uploadedDocumentTag(document.type), name: document.fileName, detail: `${Math.ceil(document.sizeBytes / 1024)} KB`, downloadLabel: downloadLabel(document.fileName, document.mimeType), documentIds: [document.id], regenerateType: null, downloadable: true })),
    ...(caseDetail.financialDocuments ?? []).map((document) => {
      const downloadable = document.status !== "cancelled";
      return { kind: "generated" as const, id: document.id, date: document.issuedAt ?? document.createdAt, label: generatedDocumentLabel(document.type), tag: generatedDocumentTag(document.type), name: document.documentNumber, detail: `${formatMoney(document.amountSen)} - ${downloadable ? titleCase(document.status) : "No longer downloadable"}`, downloadLabel: document.type === "receipt" ? "Open Document" : "Open DOCX", documentIds: downloadable ? [document.caseDocumentId, document.pdfCaseDocumentId].filter((id): id is string => Boolean(id)) : [], regenerateType: downloadable ? document.type === "quotation" ? "quotation" as const : document.type === "invoice" ? "proforma" as const : null : null, sourceId: document.sourceId, downloadable };
    }),
  ...
  return <div className="case-workspace">
    <div className="case-action-panel panel">
      ...
      <div className="case-stage-summary">
        <div className="case-stage-tracker" aria-label="Case progress">
    ...
    <ConfirmationDialog open={Boolean(dialog)} size={dialog === "resubmit" || dialog === "verify_payment" || dialog === "verify_savings" ? "lg" : "md"} title={dialog === "resubmit" ? "Update Case Details" : dialog === "reject_payment" ? "Reject Payment?" : dialog === "request_reschedule" ? "Request Installation Reschedule" : dialog === "delete_case" ? "Delete This Case?" : dialog === "cancel" ? "Cancel This Case?" : dialog === "request_changes" ? "Request Changes?" : dialog === "verify_payment" ? "Verify Payment" : dialog === "propose_installation" ? "Set Installation Date and Time" : dialog === "confirm_installation" ? "Confirm Installation Date?" : dialog === "record_installation" ? "Confirm Installation Completed?" : dialog === "verify_savings" ? "Record Savings" : dialog === "accept_trial" ? "Start Recurring Balance" : "Start Quotation Details"} description={dialog === "reject_payment" ? "Explain why the payment was rejected. The agent can then upload a new proof." : dialog === "request_reschedule" ? "Tell staff the client’s preferred date or time. Staff will enter the replacement schedule." : dialog === "verify_payment" ? "Review and allocate the pending payment before marking it as valid." : dialog === "propose_installation" ? "Set the installation date and time for the agent to confirm with the client." : dialog === "record_installation" ? "Confirm that installation was completed at the accepted date and time." : dialog === "verify_savings" ? "Enter three completed historical months." : dialog === "accept_trial" ? "Choose the first recurring instalment date and payment term before starting the schedule." : ""} confirmLabel={dialog === "reject_payment" ? "Reject Payment" : dialog === "request_reschedule" ? "Send Suggestion" : dialog === "verify_payment" ? "Verify Payment" : dialog === "propose_installation" ? "Propose Date and Time" : dialog === "record_installation" ? "Confirm Installation" : dialog === "verify_savings" ? "Record Savings" : dialog === "accept_trial" ? "Start Recurring Balance" : "Confirm"} confirmVariant={dialog === "delete_case" || dialog === "cancel" || dialog === "reject_payment" ? "danger" : "primary"} loading={busy} confirmDisabled={(dialog === "verify_payment" && Boolean(allocationError)) || (dialog === "propose_installation" && (!installationDate || !installationTime)) || (dialog === "record_installation" && (!caseDetail.installationProposedDate || !caseDetail.installationProposedTime))} reason={reason} reasonRequired={dialog === "cancel" || dialog === "request_changes" || dialog === "request_reschedule" || dialog === "reject_payment"} onReasonChange={dialog === "cancel" || dialog === "request_changes" || dialog === "request_reschedule" || dialog === "reject_payment" ? setReason : undefined} onCancel={closeDialog} onConfirm={() => dialog && run(dialog)} footer={dialog === "confirm_installation" ? <div className="dialog-actions"><Button variant="secondary" onClick={closeDialog} disabled={busy}>Cancel</Button><Button variant="danger" onClick={() => { setReason(""); setDialog("request_reschedule"); }} disabled={busy}>Request Reschedule</Button><Button variant="primary" onClick={() => run("confirm_installation")} disabled={busy}>Confirm</Button></div> : dialog === "verify_payment" ? <div className="dialog-actions"><Button variant="secondary" onClick={closeDialog} disabled={busy}>Cancel</Button>{user.role !== "agent" && <Button variant="danger" onClick={() => { setReason(""); setDialog("reject_payment"); }} disabled={busy}>Reject Payment</Button>}<Button variant="primary" onClick={() => run("verify_payment")} disabled={busy || Boolean(allocationError)}>Verify Payment</Button></div> : undefined}>
      ...
      {dialog === "verify_savings" && <div className="case-dialog-fields savings-verification-fields"><div className="proposal-readings-table savings-readings-table"><div className="proposal-reading-row proposal-reading-header" aria-hidden="true"><span /> <span>Month</span><span>Year</span><span>Bill (RM)</span><span>kWh Used</span><span>TNB Rate</span><span>Days</span></div>{savingsReadings.map((reading, index) => { const days = daysForMonth(reading.month); const bill = Number(reading.bill); const kwh = Number(reading.kwh); const rate = bill > 0 && kwh > 0 ? (bill / kwh).toFixed(6) : "—"; return <div className="proposal-reading-row" key={index}><span className="case-savings-entry-number">{index + 1}</span><FilterSelect id={`savings-month-${index + 1}`} ariaLabel={`Month ${index + 1}`} allLabel="Select month" value={reading.monthNumber} options={savingsMonthNumberOptions} labels={savingsMonthNumberLabels} onChange={(value) => { setSavingsError(null); setSavingsReadings((current) => updateSavingsReading(current, index, { monthNumber: value })); }} required /><TextInput id={`savings-year-${index + 1}`} aria-label={`Year ${index + 1}`} placeholder="YYYY" inputMode="numeric" maxLength={4} value={reading.year} onChange={(event) => { const year = event.target.value.replace(/\D/g, "").slice(0, 4); setSavingsError(null); setSavingsReadings((current) => updateSavingsReading(current, index, { year })); }} required /><MoneyInput title="TNB Bill" aria-label={`Bill amount in RM ${index + 1}`} inputMode="decimal" value={reading.bill} onChange={(event) => { setSavingsError(null); setSavingsReadings((current) => updateSavingsReading(current, index, { bill: event.target.value })); }} aria-invalid={Boolean(savingsError)} aria-describedby={savingsError ? "verify-savings-error" : undefined} required /><TextInput title="kWh Used" aria-label={`kWh used ${index + 1}`} placeholder="e.g. 2600" inputMode="decimal" value={reading.kwh} onChange={(event) => { setSavingsError(null); setSavingsReadings((current) => updateSavingsReading(current, index, { kwh: event.target.value })); }} aria-invalid={Boolean(savingsError)} aria-describedby={savingsError ? "verify-savings-error" : undefined} required /><span className="proposal-reading-calculated" aria-label={`Calculated TNB rate ${index + 1}`}>{rate}</span><span className="proposal-reading-calculated" aria-label={`Calendar days ${index + 1}`}>{days ?? "—"}</span></div>; })}</div>{savingsError && <p id="verify-savings-error" className="case-field-error-message" role="alert">{savingsError}</p>}</div>}
    ...
    <PopupModal open={detailsInfoOpen} title="Editing accepted case details" description="These corrections update the live customer and case record only." size="sm" tone="neutral" onClose={() => setDetailsInfoOpen(false)} footer={<div className="dialog-actions"><Button type="button" variant="secondary" onClick={() => setDetailsInfoOpen(false)}>Cancel</Button><Button type="button" variant="primary" onClick={() => { setDetailsInfoOpen(false); setEditing(true); }}>Continue to Edit</Button></div>}>
      <p>Accepted proposals and invoices that have already been issued are immutable. They will not be changed or regenerated after you save these corrections.</p>
    </PopupModal>
