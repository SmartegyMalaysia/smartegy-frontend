...
export function DatePicker({ id, title, value, mode = "date", placeholder, surface = "subtle", onChange, required = false, ariaLabel, ariaInvalid = false, ariaDescribedBy, displayValueOverride, range, onRangeChange, fieldClassName = "", disabled = false }: { id: string; title?: string; value: string; mode?: DatePickerMode; placeholder?: string; surface?: "subtle" | "white"; onChange: (value: string) => void; required?: boolean; ariaLabel?: string; ariaInvalid?: boolean; ariaDescribedBy?: string; displayValueOverride?: string; range?: DateRangeValue; onRangeChange?: (value: DateRangeValue) => void; fieldClassName?: string; disabled?: boolean }) {
  ...
  const estimatedPopoverHeight = yearPickerOpen ? 270 : mode === "month" ? 220 : 330;
  const viewportHeight = typeof window === "undefined" ? 0 : window.innerHeight;
  const viewportWidth = typeof window === "undefined" ? 0 : window.innerWidth;
  const fitsBelow = Boolean(rect && (!viewportHeight || rect.bottom + 8 + estimatedPopoverHeight <= viewportHeight - 8));
  const top = rect ? fitsBelow ? rect.bottom + 8 : Math.max(8, rect.top - estimatedPopoverHeight - 8) : undefined;
  const left = rect && viewportWidth ? Math.min(Math.max(8, rect.left), Math.max(8, viewportWidth - 300)) : rect?.left;
  const popover = <div className={`date-picker-popover date-picker-portal ${insidePopupModal ? "popup-modal-popover" : ""} ${popoverExiting ? "date-picker-portal-exiting" : ""}`.trim()} role="dialog" aria-label={rangeMode ? "Choose date range" : mode === "month" ? "Choose payment month" : "Choose date"} style={rect ? { top, left, transformOrigin: fitsBelow ? "top left" : "bottom left" } : undefined}>
  ...
  const picker = <details id={id} className={`date-picker date-picker-${surface} ${disabled ? "date-picker-disabled" : ""}`} ref={detailsRef} open={disabled ? false : open} onToggle={(event) => { if (disabled) { event.currentTarget.open = false; return; } const isOpen = event.currentTarget.open; setOpen(isOpen); if (isOpen) window.dispatchEvent(new CustomEvent("smartegy:popover-open", { detail: popoverId.current })); }}><summary ref={summaryRef} aria-label={ariaLabel ?? `${mode === "month" ? "Payment month" : "Date"}: ${displayValue}`} aria-invalid={ariaInvalid} aria-describedby={ariaDescribedBy} aria-disabled={disabled} tabIndex={disabled ? -1 : undefined} onClick={(event) => { if (disabled) event.preventDefault(); }}><span className={value || (rangeMode && range?.end) ? "date-picker-value" : "date-picker-placeholder"}>{displayValue}</span><span className="date-picker-icon"><CalendarIcon /></span></summary>{popoverMounted && !disabled && typeof document !== "undefined" ? createPortal(popover, document.body) : null}</details>;
  return title ? <FormField title={title} htmlFor={id} required={required} className={fieldClassName}>{picker}</FormField> : picker;
}
