...
export function DatePicker({ id, value, mode = "date", placeholder, surface = "subtle", onChange }: { id: string; value: string; mode?: DatePickerMode; placeholder?: string; surface?: "subtle" | "white"; onChange: (value: string) => void }) {
  ...
  const year = viewDate.getUTCFullYear(); const month = viewDate.getUTCMonth();
  ...
  const daysInMonth = new Date(Date.UTC(year, month + 1, 0)).getUTCDate(); const firstDay = new Date(Date.UTC(year, month, 1)).getUTCDay();
  ...
  const popover = <div className="date-picker-popover date-picker-portal" role="dialog" aria-label={mode === "month" ? "Choose payment month" : "Choose date"} style={rect ? { top: rect.bottom + 8, left: rect.left } : undefined}><div className="date-picker-toolbar"><button type="button" aria-label="Previous month" onClick={() => moveMonth(-1)}>‹</button><div className="date-picker-toolbar-heading"><span>{monthNames[month]}</span><button className="date-picker-year-trigger" type="button" aria-expanded={yearPickerOpen} onClick={() => setYearPickerOpen((current) => !current)}>{year}</button></div><button type="button" aria-label="Next month" onClick={() => moveMonth(1)}>›</button></div>{yearPickerOpen ? <div className="date-picker-year-picker" aria-label="Choose year"><div className="date-picker-year-toolbar"><button type="button" aria-label="Previous year range" onClick={() => moveYear(-12)}>‹</button><span>{years[0]}–{years[years.length - 1]}</span><button type="button" aria-label="Next year range" onClick={() => moveYear(12)}>›</button></div><div className="date-picker-year-grid">{years.map((option) => <button className={option === year ? "date-picker-selected" : ""} key={option} type="button" onClick={() => selectYear(option)}>{option}</button>)}</div></div> : mode === "month" ? <div className="date-picker-month-grid">{shortMonthNames.map((name, index) => <button className={value === `${year}-${String(index + 1).padStart(2, "0")}` ? "date-picker-selected" : ""} key={name} type="button" onClick={() => selectMonth(index)}>{name}</button>)}</div> : <div className="date-picker-day-grid">{weekDays.map((day) => <span className="date-picker-weekday" key={day}>{day}</span>)}{days.map((day, index) => day ? <button className={value === `${year}-${String(month + 1).padStart(2, "0")}-${String(day).padStart(2, "0")}` ? "date-picker-selected" : ""} key={index} type="button" onClick={() => selectDay(day)}>{day}</button> : <span key={index}/>)}</div>}</div>;
  return <details className={`date-picker date-picker-${surface}`} ref={detailsRef} open={open} onToggle={(event) => { const isOpen = event.currentTarget.open; setOpen(isOpen); if (isOpen) window.dispatchEvent(new CustomEvent("smartegy:popover-open", { detail: popoverId.current })); }}><summary ref={summaryRef} aria-label={`${mode === "month" ? "Payment month" : "Date"}: ${displayValue}`}><span className={value ? "date-picker-value" : "date-picker-placeholder"}>{displayValue}</span><span className="date-picker-icon"><CalendarIcon /></span></summary>{open && typeof document !== "undefined" ? createPortal(popover, document.body) : null}</details>;
}
