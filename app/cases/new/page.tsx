...
import { malaysiaIndustries } from "@/lib/malaysia-industries";
...
export default function NewCasePage() {
  ...
  const [customBusinessTypeError, setCustomBusinessTypeError] = useState<string | null>(null);
  ...
  function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    setFieldError(null); setBusinessTypeError(null); setCustomBusinessTypeError(null); setBillError(null); setToast(null); setPermissionDenied(false);
    let invalid = false;
    if (!customerName.trim()) { setFieldError("Enter the customer or company name."); invalid = true; }
    if (!businessType) { setBusinessTypeError("Select the type of business."); invalid = true; }
    if (businessType === "Other" && !customBusinessType.trim()) { setCustomBusinessTypeError("Enter the type of business."); invalid = true; }
    if (!billFiles.length) { setBillError("Upload the latest electricity bill before submitting."); invalid = true; }
    if (invalid) return;
...
function Field({ id, label, value, onChange, type = "text", required = false, optional = false, autoComplete, error }: { id: string; label: string; value: string; onChange: (value: string) => void; type?: string; required?: boolean; optional?: boolean; autoComplete?: string; error?: string }) { return <FormField title={label} htmlFor={id} required={required} className={error ? "case-field-error" : ""}><TextInput id={id} type={type} value={value} onChange={(event) => onChange(event.target.value)} autoComplete={autoComplete} required={required} aria-invalid={Boolean(error)} aria-describedby={error ? `${id}-error` : undefined} />{error && <p id={`${id}-error`} className="field-error" role="alert">{error}</p>}</FormField>; }
