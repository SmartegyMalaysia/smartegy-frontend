...
function SelectField({ id, label, value, onChange, options, required = false, optional = false, error }: { id: string; label: string; value: string; onChange: (value: string) => void; options: readonly string[]; required?: boolean; optional?: boolean; error?: string }) { const fieldOptions = label === "State" ? malaysiaStates : options; return <FormField title={label} htmlFor={id} required={required} className={error ? "case-field-error" : ""}><FilterSelect id={id} allLabel={`Select ${label.toLowerCase()}`} ariaLabel={label} ariaInvalid={Boolean(error)} value={value} options={[...fieldOptions]} onChange={onChange} />{error && <p id={`${id}-error`} className="field-error" role="alert">{error}</p>}</FormField>; }
