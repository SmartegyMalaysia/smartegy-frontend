...
function DashboardContent({ snapshot, role }: { snapshot: DashboardSnapshot; role: UserRole }) {
  ...
  return <>
    {isAgent ? <><>{/* Current qualification is intentionally hidden from the home dashboard. */}</><div className="stat-grid"><StatCard label="Active cases" value={String(activeCases.length)} detail="Across 2 statuses" accent /><StatCard label="Successful cases" value="8" detail="+2 this quarter" /><StatCard label="Personal sales" value={formatMoney(11240000)} detail="Year to date" /><StatCard label="Commissions earned" value={formatMoney(commissions)} detail="1 payment scheduled" /></div></> : <><section className="focus-panel admin-focus"><div><p className="eyebrow">Operational priority</p><h2>Cases needing attention</h2><p className="focus-copy">2 cases and 1 payment are waiting for an update.</p><div className="priority-list"><span><i className="priority-dot warning" />Pending review <strong>2</strong></span><span><i className="priority-dot danger" />Payment verification <strong>1</strong></span></div></div><div className="focus-side"><span className="priority-number">03</span><span>Open priorities</span><Link href="/cases">Open queue <Icon name="arrow" size={14} /></Link></div></section><div className="stat-grid"><StatCard label="Total cases" value="128" detail="+12 this month" accent /><StatCard label="Sales this month" value={formatMoney(sales)} detail="Across 4 cases" /><StatCard label="Commission payable" value={formatMoney(468320)} detail="18 entries" /><StatCard label="Paid commissions" value={formatMoney(892140)} detail="This quarter" /></div></>}
...
function TeamSnapshot({ agents }: { agents: DashboardSnapshot["agents"] }) {
  return <section className="panel side-panel"><div className="panel-header"><div><h2>Team snapshot</h2><p>Agent performance</p></div><Link className="text-link" href="/agents">View all <Icon name="arrow" size={14} /></Link></div><div className="team-list">{agents.map((agent) => <div className="team-item" key={agent.id}><span className="avatar avatar-small">{agent.displayName.split(" ").map((name) => name[0]).join("")}</span><div><strong>{agent.displayName}</strong><span>Level {agent.currentLevel} · {agent.successfulCaseCount} successful cases</span></div><span className="team-sales">{formatMoney(agent.personalSalesSen)}</span></div>)}</div></section>;
}
