...
function ProfileContent({
  ...
  useEffect(() => {
    setReferralLink(`${window.location.origin}/join/${profile.referralCode}`);
  }, [profile.referralCode]);
  async function sendVerification() {
    const result = await agentProfileRepository.requestEmailVerification(user);
    if (result.ok) {
      setVerificationSent(true);
      setFeedback({
        tone: "success",
        message:
          "Verification instructions have been sent to your new email address.",
      });
    } else setFeedback({ tone: "error", message: result.error.message });
  }
  async function copyReferral(value: string, kind: "link" | "code") {
    try {
      await navigator.clipboard.writeText(value);
      setCopiedReferral(kind);
      setTimeout(() => setCopiedReferral(null), 1800);
    } catch {
      setFeedback({
        tone: "error",
        message:
          "The referral value could not be copied. Select it and copy it manually.",
      });
    }
  }
  return (
      ...
      <div className="page-header profile-page-header">
        <div>
          ...
          <p className="page-description">
            Keep your personal contact details current and review your confirmed
            account record.
          </p>
        </div>
      </div>
      {pending && <PendingNotice profile={profile} />}
      {feedback && (
        <div
      ...
      <div className="profile-layout">
        <section className="panel profile-identity-panel">
          <div className="profile-identity">
            ...
            <div>
              <p className="eyebrow">
                {pending ? "Pending agent" : "Verified agent"}
              </p>
          ...
          <div className="profile-status-stack">
            <span>
              Account <Badge status={profile.accountStatus} />
            </span>
            <span>
              Email{" "}
              <Badge
                status={emailVerified ? "verified" : "pending_verification"}
              />
            </span>
            <span>
              Registration <Badge status={profile.registrationStatus} />
            </span>
            <span>
              Fee <Badge status={profile.feeStatus} />
            </span>
          </div>
        </section>
        {!pending && (
          <section className="panel profile-form-panel">
            <div className="panel-header">
              <div>
                ...
                <p>
                  Only your name, mobile number, and email address can be
                  changed.
                </p>
              </div>
            </div>
            <form className="profile-form" onSubmit={save} noValidate>
              ...
              />
              {emailChanged && (
                <p className="profile-inline-note" role="status">
                  Changing your email will require verification again before the
                  new address is treated as verified.
                </p>
              )}
            ...
            {!emailVerified && !emailChanged && (
              <div className="profile-verification-panel">
                <p>
                  ...
                  <span>
                    Verification is required before this address is treated as
                    verified.
                  </span>
                </p>
                {!verificationSent ? (
                  ...
                  <div className="profile-verification-form">
                    ...
                    <small>Use the verification code from the email.</small>
                  </div>
                )}
              </div>
            )}
        ...
        {!pending && (
          <section className="panel profile-form-panel">
            <div className="panel-header">
              <div>
                <h2>Bank details</h2>
                <p>These details are used for commission payouts.</p>
              </div>
            </div>
            <form
            ...
            >
              ...
              <ProfileField
                ...
                inputMode="numeric"
              />
              <p className="profile-inline-note">Please confirm these details carefully. Future commission payouts will use this account.</p>
              <div className="profile-form-actions">
                <Button type="submit" disabled={bankSaving || !bankChanged}>
                  {bankSaving ? "Saving bank detailsâ€¦" : "Save bank details"}
                </Button>
                {bankChanged && <span>Unsaved changes</span>}
              </div>
            </form>
          </section>
        )}
      ...
      {!pending && (
        <section className="panel profile-actions-panel">
          <div>
            ...
            <p>
              Password controls are handled separately from your profile
              details.
            </p>
          </div>
