...
function bankSelectionFor(bankName: string) {
  if (!bankName) return "";
  return MALAYSIAN_BANKS.some((bank) => bank === bankName)
    ? bankName
    : OTHER_BANK_OPTION;
}
...
function ProfileContent({
  ...
  const [bankSelection, setBankSelection] = useState(() =>
    bankSelectionFor(profile.bankDetails?.bankName ?? ""),
  );
  const [otherBankName, setOtherBankName] = useState(() =>
    bankSelectionFor(profile.bankDetails?.bankName ?? "") === OTHER_BANK_OPTION
  ...
  function updateBankSelection(value: string) {
    setBankSelection(value);
    updateBank(
      "bankName",
      value === OTHER_BANK_OPTION ? otherBankName : value,
    );
  }
  function updateOtherBankName(value: string) {
  ...
  async function saveBankDetails(event: React.FormEvent) {
    ...
    setBankForm(next);
    const nextBankSelection = bankSelectionFor(next.bankName);
    setBankSelection(nextBankSelection);
    setOtherBankName(
      nextBankSelection === OTHER_BANK_OPTION ? next.bankName : "",
    );
    setFieldErrors({});
    setFeedback({
      tone: "success",
