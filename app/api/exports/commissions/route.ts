...
export async function GET(request: NextRequest) {
  ...
  const rows = [["Case", "Customer", "Entitlement", "Paid", "Remaining", "Status", "Next payout", "Updated"], ...(payload.items ?? []).map((item: any) => [item.case_number, item.customer_name, item.amount, item.status === "paid" ? item.amount : 0, item.status === "paid" ? 0 : item.amount, item.status, item.status === "withheld" ? 0 : item.amount, item.paid_at ?? item.due_date])];
