...
export async function GET(request: NextRequest) {
  ...
  const rows = [["Case", "Customer", "Entitlement", "Paid", "Remaining", "Status", "Next payout", "Updated"], ...records.map((item) => [item.caseNumber, item.customerDisplayName, item.entitlementSen / 100, item.paidToDateSen / 100, item.deferredBalanceSen / 100, item.status, item.nextPaymentDate ? formatDate(item.nextPaymentDate) : "", formatDateTime(item.lastUpdatedAt)])];
