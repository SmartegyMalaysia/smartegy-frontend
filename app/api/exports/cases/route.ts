...
export async function GET(request: NextRequest) {
  ...
  const rows = [["Case", "Customer", "Agent", "Amount", "Status", "Payment", "Updated"], ...(payload.items ?? []).map((item: any) => [item.case_number, item.customer_name, item.agent_name, item.sale_amount, item.status, item.payment_status, formatDateTime(item.status_changed_at ?? item.created_at)])];
