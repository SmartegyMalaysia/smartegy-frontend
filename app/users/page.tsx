...
export default function UsersPage() {
  ...
  const [createForm, setCreateForm] = useState<CreateStaffInput>({ displayName: "", email: "", phone: "" });
  ...
  function openCreateStaff() { setCreateForm({ displayName: "", email: "", phone: "" }); setCreateErrors({}); setCreateFeedback(null); setCreating(true); }
  const closeCreateStaff = useCallback(() => { if (!creatingStaff) { setCreating(false); setCreateErrors({}); setCreateFeedback(null); } }, [creatingStaff]);
  const closeEditor = useCallback(() => { if (!saving) { setEditing(null); setFieldErrors({}); setEditFeedback(null); } }, [saving]);
  async function saveUser(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!editing) return;
    if (editing.role !== form.role && (editing.role === "agent" || form.role === "agent")) {
      setFieldErrors({ role: ["Agent accounts cannot be converted to or from Staff or Administrator."] });
      setEditFeedback("Agent accounts cannot be converted to or from Staff or Administrator.");
      return;
    }
  ...
  async function createStaff(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    setCreatingStaff(true); setCreateFeedback(null);
    const result = await userRepository.createStaff(user, createForm);
    setCreatingStaff(false);
    if (!result.ok) { setCreateErrors(result.error.fieldErrors ?? {}); setCreateFeedback(result.error.message); return; }
    setCreating(false); setFeedback(`An invitation was sent to ${result.data.email}.`);
    setPage(1); void load();
  }
  ...
  return <AppShell user={user} onRoleChange={setRole}><main className="page-content users-page">
    <div className="page-header"><div><p className="eyebrow">Administration</p><h1>Users</h1><p className="page-description">Keep account access, roles, and contact details accurate across Smartegy.</p></div><div className="users-header-actions">{user.role === "admin" && <Button onClick={openCreateStaff}><Icon name="plus" size={16}/>Create staff</Button>}<div className="users-access-note"><Icon name="user-settings" size={16}/><span>Admin access only</span></div></div></div>
...
function CreateStaffEditor({ form, setForm, fieldErrors, feedback, saving, onClose, onSave }: { form: CreateStaffInput; setForm: React.Dispatch<React.SetStateAction<CreateStaffInput>>; fieldErrors: Record<string, string[]>; feedback: string | null; saving: boolean; onClose: () => void; onSave: (event: React.FormEvent<HTMLFormElement>) => void }) {
  ...
  return <PopupModalLayerContext.Provider value><div className="user-editor-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section className="user-editor" role="dialog" aria-modal="true" aria-labelledby="create-staff-title" aria-describedby="create-staff-description"><header className="user-editor-header"><div><p className="eyebrow">User access</p><h2 id="create-staff-title">Create Staff</h2><p id="create-staff-description">Send a staff invitation. They will set their own password from the email link before gaining access.</p></div><button ref={closeRef} className="dialog-close" type="button" aria-label="Close create staff panel" onClick={onClose}><Icon name="close" size={18}/></button></header><form className="user-editor-form" onSubmit={onSave} noValidate><div className="user-editor-fields"><EditorField id="staff-display-name" label="Display name" value={form.displayName} onChange={(value) => setForm((current) => ({ ...current, displayName: value }))} error={fieldErrors.displayName?.[0]} autoComplete="name"/><EditorField id="staff-email" label="Work email" value={form.email} onChange={(value) => setForm((current) => ({ ...current, email: value }))} error={fieldErrors.email?.[0]} autoComplete="email" type="email"/><div className={`user-editor-field ${fieldErrors.phone ? "user-editor-field-error" : ""}`}><label htmlFor="staff-phone">Phone number <span className="muted-cell">(optional)</span></label><TextInput id="staff-phone" value={form.phone} onChange={(event) => setForm((current) => ({ ...current, phone: event.target.value }))} autoComplete="tel" placeholder="e.g. +60 12-345 6789" aria-invalid={Boolean(fieldErrors.phone)}/>{fieldErrors.phone?.[0] && <p className="field-error" role="alert">{fieldErrors.phone[0]}</p>}</div><div className="user-editor-invitation-note"><RoleBadge role="staff"/><p>The account starts as Invited. After the recipient creates a password from the invitation, it is activated automatically.</p></div></div>{feedback && <div className="user-editor-feedback" role="alert">{feedback}</div>}<footer className="user-editor-actions"><Button type="button" variant="secondary" onClick={onClose} disabled={saving}>Cancel</Button><Button type="submit" disabled={saving}>{saving ? "Sending invitation…" : "Send invitation"}</Button></footer></form></section></div></PopupModalLayerContext.Provider>;
}
...
function UserEditor({ user, actor, form, setForm, fieldErrors, feedback, saving, onClose, onSave }: { user: ManageUser; actor: { id: string; role: UserRole; accountStatus?: AccountStatus }; form: UpdateManageUserInput; setForm: React.Dispatch<React.SetStateAction<UpdateManageUserInput>>; fieldErrors: Record<string, string[]>; feedback: string | null; saving: boolean; onClose: () => void; onSave: (event: React.FormEvent<HTMLFormElement>) => void }) {
  ...
  const activationBlocked = user.role === "agent" && Boolean(user.accountActivationBlocked) && isInactive;
  const statusActionLabel = isInactive ? "Enable account" : "Disable account";
  const statusDescription = activationBlocked ? "This agent’s registration fee has not been verified. Verify or waive the fee in Registrations before enabling the account." : isInactive ? "This account cannot sign in until it is enabled." : form.accountStatus === "invited" ? "This invitation can be disabled before the recipient completes setup." : "This account can sign in and access its permitted workspace.";
