...
function AgentRow({ agent }: { agent: AgentSummary }) {
  ...
  const registrationPending = agent.registrationStatus === "pending_approval";
  const eligibility = registrationPending ? "Pending Admin Approval" : qualification.nextLevel === null ? "Highest Level" : qualification.eligibleForPromotion ? "Eligible for Promotion" : "In Progress";
  return <tr className="agent-table-row case-table-row" tabIndex={0} role="link" aria-label={`Open details for ${agent.displayName}`} onClick={open} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); open(); } }}><td><span className="table-primary">{agent.displayName} <Badge status={agent.status}/>{registrationPending && <Badge status="pending_approval"/>}</span><span className="table-secondary">{agent.agentCode}</span></td><td>Level {agent.currentLevel}</td><td>{agent.uplineName ?? <span className="muted-cell">Direct registration</span>}</td><td>{agent.successfulCaseCount}</td><td>{agent.directAgentCount}</td><td className="commission-money">{formatMoney(agent.personalSalesSen)}</td><td className="commission-money">{formatMoney(agent.referralSalesSen)}</td><td className="commission-money">{formatMoney(agent.commissionEarnedSen)}</td><td><span className={`qualification-status ${registrationPending ? "qualification-pending" : qualification.eligibleForPromotion ? "qualification-ready" : qualification.nextLevel === null ? "" : "qualification-in-progress"}`}>{eligibility}</span></td></tr>;
}
