...
const mockUsers: ManageUser[] = [
  ...
  { id: "user-004", displayName: "Daniel Lim", email: "daniel@smartegy.example", phone: "+60 16-445 2301", role: "agent", accountStatus: "inactive", agentCode: "AG-002", lastActiveAt: "2026-08-16T08:05:00Z", createdAt: "2026-06-03T04:50:00Z" },
...
export const mockUserRepository: UserRepository = {
  ...
  async update(actor, userId, input) {
    ...
    if (input.role !== user.role && (user.role === "agent" || input.role === "agent")) return fail("CONFLICT", "Agent accounts cannot be converted to or from Staff or Administrator.", { role: ["Agent accounts cannot be converted to or from Staff or Administrator."] });
