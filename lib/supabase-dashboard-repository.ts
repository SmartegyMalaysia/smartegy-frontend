...
import type { CaseStatus, DashboardSnapshot, UserRole } from "./types";
...
export const supabaseDashboardRepository: DashboardRepository = {
  async getSnapshot(role: UserRole) {
    ...
    const [{ data: caseRows, error: caseError }, { data: commissionRows, error: commissionError }, { data: agentRows, error: agentError }, { data: scheduleRows, error: scheduleError }] = await Promise.all([
      ...
      supabase.from("payment_schedules").select("case_id,amount_due,amount_paid,status").limit(1000),
    ]);
    ...
    if (scheduleError) throw new Error(normalizeSupabaseError(scheduleError).message);
    const schedulesByCase = new Map<string, any[]>(); (scheduleRows ?? []).forEach((schedule: any) => { const list = schedulesByCase.get(schedule.case_id) ?? []; list.push(schedule); schedulesByCase.set(schedule.case_id, list); });
    const cases = (caseRows ?? []).map((row: any) => { const schedules = schedulesByCase.get(row.id) ?? []; return { id: row.id, caseNumber: row.case_number, customerDisplayName: row.customer_name, agentId: row.agent_id, agentName: row.agent_name, status: caseStatus(row.status), paymentStatus: schedules.length ? (Number(row.outstanding_customer_balance ?? 0) <= 0 ? "verified" : "pending_verification") : "not_recorded", saleAmountSen: row.sale_amount == null ? null : money(row.sale_amount), submittedAt: row.created_at, updatedAt: row.status_changed_at }; });
