...
function validateBankDetails(input: UpdateAgentBankDetailsInput) { const fieldErrors: Record<string, string[]> = {}; if (input.bankName.trim().length < 2) fieldErrors.bankName = ["Enter the bank name."]; if (input.accountHolderName.trim().length < 2) fieldErrors.accountHolderName = ["Enter the account holder name."]; if (input.accountNumber.trim().length < 4) fieldErrors.accountNumber = ["Enter a valid bank account number."]; return fieldErrors; }

export interface AgentProfileRepository { getMine(actor: CurrentUser): Promise<ProfileActionResult<AgentProfile>>; updateMine(actor: CurrentUser, input: UpdateAgentProfileInput & Record<string, unknown>): Promise<ProfileActionResult<AgentProfile>>; updateBankDetails(actor: CurrentUser, input: UpdateAgentBankDetailsInput): Promise<ProfileActionResult<AgentProfile>>; requestEmailVerification(actor: CurrentUser): Promise<ProfileActionResult<{ expiresInSeconds: number }>>; verifyEmail(actor: CurrentUser, code: string): Promise<ProfileActionResult<AgentProfile>>; }
