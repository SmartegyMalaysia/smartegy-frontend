...
export const mockCasesRepository: CasesRepository = {
  ...
  async rejectPayment(actor, paymentId, reason) { const permission = staffOnly<CaseDetail>(actor); if (permission) return permission; if (!reason.trim()) return failure("VALIDATION_ERROR", "A rejection reason is required."); const found = Array.from(caseStore.values()).find((item) => item.payments?.some((payment) => payment.id === paymentId)); const payment = found?.payments?.find((item) => item.id === paymentId); if (!found || !payment || payment.status !== "pending_verification") return failure("NOT_FOUND", "Pending payment not found."); payment.status = "rejected"; payment.rejectionReason = reason.trim(); found.paymentStatus = "not_recorded"; recordActivity(found, actor, "payment_rejected", "Payment rejected.", reason.trim()); return { ok: true, data: found }; },
  ...
  async verifyPayment(actor, input) {
    ...
    owner.paymentStatus = hasPending ? "pending_verification" : "verified";
