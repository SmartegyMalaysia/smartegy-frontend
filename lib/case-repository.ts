...
export const mockCasesRepository: CasesRepository = {
  ...
  async acceptProposal(actor, caseId, input) { const permission = staffOnly<CaseDetail>(actor); if (permission) return permission; const found = caseStore.get(caseId); if (!found) return failure("NOT_FOUND", "Case not found."); if (!found.proposal || found.proposal.status !== "issued") return failure("VALIDATION_ERROR", "An issued proposal is required."); const signedDocumentId = `document-${caseId}-signed-proposal`; found.documents = [...found.documents, { id: signedDocumentId, caseId, type: "signed_proposal", fileName: input.signedProposal.name, mimeType: input.signedProposal.type || "application/pdf", sizeBytes: input.signedProposal.size, uploadedBy: actor.id, uploadedAt: now(), visibleToAgent: true }]; found.proposal.status = "accepted"; found.proposal.acceptedByName = input.acceptedByName; found.proposal.acceptanceDate = input.acceptanceDate; found.proposal.selectedTermMonths = input.selectedTermMonths; found.proposal.signedDocumentId = signedDocumentId; found.proposal.acceptedAt = now(); const deposit = found.proposal.deposit1Sen; const post = found.proposal.deposit2Sen; const depositScheduleId = `schedule-${caseId}-1`; found.paymentSchedules = [{ id: depositScheduleId, caseId, sequence: 1, kind: "deposit", dueDate: input.acceptanceDate, amountDueSen: deposit, amountPaidSen: 0, status: "scheduled" }, { id: `schedule-${caseId}-2`, caseId, sequence: 2, kind: "post_installation", dueDate: input.acceptanceDate, amountDueSen: post, amountPaidSen: 0, status: "scheduled" }]; found.installmentTermMonths = input.selectedTermMonths; found.status = "awaiting_deposit"; found.paymentStatus = "pending_verification"; found.financialDocuments = [...(found.financialDocuments ?? []), { id: `document-${caseId}-proforma`, sourceId: depositScheduleId, documentNumber: `INV-MOCK-${caseId}`, type: "invoice", amountSen: deposit, issueDate: input.acceptanceDate, status: "issued", createdAt: now() }]; recordActivity(found, actor, "proposal_accepted", "Proposal accepted and deposit proforma prepared."); return { ok: true, data: found }; },
