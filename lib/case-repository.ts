...
import type { AcceptTrialInput, CaseDetail, CaseDocumentInput, CasePayment, CaseSummary, CurrentUser, CreateCaseInput, GeneratePaymentScheduleInput, ID, RecordPaymentInput, RejectPaymentInput, UpdateCaseInput, VerifyPaymentInput, CaseStatus, PaymentStatus } from "./types";
...
const allowedTransitions: Partial<Record<CaseStatus, CaseStatus[]>> = {
  ...
  awaiting_deposit: ["installation_date_proposed", "cancelled"],
  installation_date_proposed: ["installation_scheduled", "cancelled"],
...
export interface CasesRepository {
  ...
  rejectPayment(actor: CurrentUser, input: RejectPaymentInput): Promise<CaseResult<CaseDetail>>;
  proposeInstallationDate(actor: CurrentUser, caseId: ID, installationDate: ISODate): Promise<CaseResult<CaseDetail>>;
  respondToInstallationDate(actor: CurrentUser, caseId: ID, confirmed: boolean, reason?: string): Promise<CaseResult<CaseDetail>>;
...
export const mockCasesRepository: CasesRepository = {
  async listPage(actor, query) {
    ...
    const stageMatch = (item: CaseSummary) => !query.stage || (query.stage === "new" ? ["draft", "submitted"].includes(item.status) : query.stage === "under_review" ? item.status === "under_review" : query.stage === "changes_requested" ? item.status === "changes_requested" : query.stage === "quotation_payment" ? ["quotation_issued", "awaiting_deposit"].includes(item.status) : query.stage === "installation_monitoring" ? ["installation_date_proposed", "installation_scheduled", "installed_monitoring"].includes(item.status) : query.stage === "trial_review" ? item.status === "trial_review" : query.stage === "commission_active" ? item.status === "active_installments" : ["completed", "cancelled"].includes(item.status));
  ...
  async transition(actor, caseId, to, reason) {
    ...
    if ((to === "installation_date_proposed" || to === "installation_scheduled") && found.paymentSchedules?.find((schedule) => schedule.kind === "deposit")?.status !== "paid") return failure("VALIDATION_ERROR", "The deposit must be verified before installation can be scheduled.");
    if (to === "installation_scheduled" && (!found.installationDate || !found.installationConfirmedBy)) return failure("VALIDATION_ERROR", "The agent must confirm the proposed installation date before scheduling installation.");
  ...
  async submitDeposit(actor, caseId, input) {
    if (actor.role !== "agent") return failure("FORBIDDEN", "Only the submitting agent can record the deposit.");
    const found = caseStore.get(caseId); if (!found) return failure("NOT_FOUND", "Case not found.");
    if (!access(actor, found)) return failure("FORBIDDEN", "You can only access your own cases.");
    if (found.status !== "awaiting_deposit") return failure("VALIDATION_ERROR", "A deposit can only be recorded while the case is awaiting deposit.");
    const schedule = found.paymentSchedules?.find((item) => item.kind === "deposit");
    const balance = schedule ? schedule.amountDueSen - schedule.amountPaidSen : 0;
    if (!schedule || input.amountSen !== balance) return failure("VALIDATION_ERROR", "Record the exact outstanding deposit amount.");
    if (found.payments?.some((payment) => payment.status === "pending_verification")) return failure("CONFLICT", "A deposit is already awaiting staff verification.");
    const payment: CasePayment = { id: `payment-${caseId}-${(found.payments?.length ?? 0) + 1}`, caseId, amountSen: input.amountSen, paymentDate: input.paymentDate, reference: input.reference ?? null, status: "pending_verification", recordedBy: actor.id, recordedAt: now(), verifiedBy: null, verifiedAt: null };
    found.payments = [...(found.payments ?? []), payment]; found.paymentStatus = "pending_verification"; recordActivity(found, actor, "deposit_submitted", "Deposit submitted and awaiting staff verification."); return { ok: true, data: found };
  },
  ...
  async verifyPayment(actor, input) { const permission = staffOnly<CaseDetail>(actor); if (permission) return permission; const owner = Array.from(caseStore.values()).find((item: CaseDetail) => item.payments?.some((payment) => payment.id === input.paymentId)); if (!owner) return failure("NOT_FOUND", "Payment not found."); const payment = owner.payments?.find((item) => item.id === input.paymentId); if (!payment || payment.status !== "pending_verification") return failure("VALIDATION_ERROR", "Payment is not pending verification."); if (input.allocations.reduce((sum, allocation) => sum + allocation.amountSen, 0) !== payment.amountSen) return failure("VALIDATION_ERROR", "Allocations must equal the payment amount."); for (const allocation of input.allocations) { const schedule = owner.paymentSchedules?.find((item) => item.id === allocation.scheduleId); if (!schedule || schedule.amountPaidSen + allocation.amountSen > schedule.amountDueSen) return failure("VALIDATION_ERROR", "Payment allocation exceeds the schedule balance."); schedule.amountPaidSen += allocation.amountSen; schedule.status = schedule.amountPaidSen === schedule.amountDueSen ? "paid" : "partially_paid"; } payment.status = "verified"; payment.verifiedBy = actor.id; payment.verifiedAt = now(); owner.paymentStatus = "verified"; recordActivity(owner, actor, "payment_verified", "Payment verified."); const installments = owner.paymentSchedules?.filter((schedule) => schedule.kind === "installment") ?? []; if (owner.status === "active_installments" && installments.length > 0 && installments.every((schedule) => schedule.status === "paid")) return this.transition(actor, owner.id, "completed", "All instalment obligations paid."); return { ok: true, data: owner }; },
  async rejectPayment(actor, input) { const permission = staffOnly<CaseDetail>(actor); if (permission) return permission; if (!input.reason.trim()) return failure("VALIDATION_ERROR", "A rejection reason is required."); const owner = Array.from(caseStore.values()).find((item) => item.payments?.some((payment) => payment.id === input.paymentId)); if (!owner) return failure("NOT_FOUND", "Payment not found."); const payment = owner.payments?.find((item) => item.id === input.paymentId); if (!payment || payment.status !== "pending_verification") return failure("VALIDATION_ERROR", "Payment is not pending verification."); payment.status = "rejected"; recordActivity(owner, actor, "payment_rejected", "Deposit rejected.", input.reason); return { ok: true, data: owner }; },
  async proposeInstallationDate(actor, caseId, installationDate) { const permission = staffOnly<CaseDetail>(actor); if (permission) return permission; const found = caseStore.get(caseId); if (!found) return failure("NOT_FOUND", "Case not found."); if (!['awaiting_deposit', 'installation_date_proposed'].includes(found.status)) return failure("VALIDATION_ERROR", "The deposit must be verified before an installation date can be proposed."); if (found.paymentSchedules?.find((schedule) => schedule.kind === "deposit")?.status !== "paid") return failure("VALIDATION_ERROR", "The deposit must be verified before an installation date can be proposed."); found.installationProposedDate = installationDate; found.installationProposedBy = actor.id; found.installationProposedAt = now(); found.installationDateFeedback = null; if (found.status === "awaiting_deposit") { found.status = "installation_date_proposed"; found.paymentStatus = "verified"; } recordActivity(found, actor, "installation_date_proposed", `Installation date proposed: ${installationDate}.`); return { ok: true, data: found }; },
  async respondToInstallationDate(actor, caseId, confirmed, reason) { if (actor.role !== "agent") return failure("FORBIDDEN", "Only the submitting agent can confirm the installation date."); const found = caseStore.get(caseId); if (!found) return failure("NOT_FOUND", "Case not found."); if (!access(actor, found)) return failure("FORBIDDEN", "You can only access your own cases."); if (found.status !== "installation_date_proposed" || !found.installationProposedDate) return failure("VALIDATION_ERROR", "There is no installation date awaiting your confirmation."); if (!confirmed && !reason?.trim()) return failure("VALIDATION_ERROR", "Tell staff why a different installation date is needed."); if (!confirmed) { found.installationDateFeedback = reason!.trim(); recordActivity(found, actor, "installation_date_change_requested", "Agent requested a different installation date.", reason!.trim()); return { ok: true, data: found }; } found.installationDate = found.installationProposedDate; found.installationConfirmedBy = actor.id; found.installationConfirmedAt = now(); found.installationDateFeedback = null; found.status = "installation_scheduled"; recordActivity(found, actor, "installation_date_confirmed", `Installation date confirmed: ${found.installationDate}.`); return { ok: true, data: found }; },
  async recordInstallation(actor, caseId, installationDate) { const permission = staffOnly<CaseDetail>(actor); if (permission) return permission; const found = caseStore.get(caseId); if (!found) return failure("NOT_FOUND", "Case not found."); if (found.status !== "installation_scheduled" || !found.installationConfirmedBy || found.installationDate !== installationDate) return failure("VALIDATION_ERROR", "The agent-confirmed installation date is required."); found.monitoringStartedOn = installationDate; const result = await this.transition(actor, caseId, "installed_monitoring", "Installation completed"); return result; },
