...
export interface CasesRepository {
  ...
  regenerateProposal(actor: CurrentUser, caseId: ID, input: ProposalInput): Promise<CaseResult<CaseDetail>>;
...
export const mockCasesRepository: CasesRepository = {
  ...
  async saveProposalDraft(actor, caseId, input) {
    const permission = staffOnly<CaseDetail>(actor); if (permission) return permission;
    const found = caseStore.get(caseId); if (!found) return failure("NOT_FOUND", "Case not found.");
    if (!["under_review", "changes_requested"].includes(found.status)) return failure("VALIDATION_ERROR", "A proposal can only be prepared for a case under review.");
    const calculated = calculateProposalPreview(input); if (!calculated) return failure("VALIDATION_ERROR", "Enter between one and twelve valid completed monthly readings and a valid project amount.");
    const previousProposal = found.proposal;
    const isReplacement = previousProposal?.status === "void";
    const version = isReplacement ? (previousProposal?.version ?? 0) + 1 : previousProposal?.version ?? 1;
    found.proposal = { id: isReplacement ? `proposal-${caseId}-${version}` : previousProposal?.id ?? `proposal-${caseId}`, caseId, version, reference: null, status: "draft", proposalDate: input.proposalDate, salesRepName: input.salesRepName, installationAddress: input.installationAddress, installationCostSen: input.installationCostSen, outstationCostSen: input.outstationCostSen, saleAmountSen: calculated.saleAmountSen, calculatedDownpaymentSen: calculated.calculatedDownpaymentSen, deposit1Sen: calculated.downpaymentSen, deposit2Sen: calculated.postInstallationSen, downpaymentTotalSen: calculated.downpaymentTotalSen, balanceSen: calculated.balanceSen, financingInterestSen: calculated.financingInterestSen, option1TotalSen: calculated.option1TotalSen, option2TotalSen: calculated.option2TotalSen, option1MonthlySen: calculated.option1MonthlySen, option2MonthlySen: calculated.option2MonthlySen, avgRate: calculated.avgRate, avgKwh: calculated.avgKwh, avgBillSen: calculated.avgBillSen, avgDayKwh: calculated.avgDayKwh, beforeInstallKwh: calculated.beforeInstallKwh, afterInstallKwh: calculated.afterInstallKwh, savingKwhMonth: calculated.savingKwhMonth, savingRmMonthSen: calculated.savingRmMonthSen, savingRmYearSen: calculated.savingRmYearSen, savingRm2YSen: calculated.savingRm2YSen, savingRm15YSen: calculated.savingRm15YSen, acceptedByName: null, acceptanceDate: null, selectedTermMonths: null, signedDocumentId: null, issuedAt: null, acceptedAt: null };
    found.proposalReadings = input.readings;
    recordActivity(found, actor, "proposal_draft_saved", "Proposal draft saved.");
    return { ok: true, data: found };
  },
  async issueProposal(actor, caseId, input) {
    const permission = staffOnly<CaseDetail>(actor); if (permission) return permission;
    const draft = await this.saveProposalDraft(actor, caseId, input); if (!draft.ok) return draft;
    const found = draft.data; if (!found.proposal) return failure("INTERNAL_ERROR", "Proposal draft was not created.");
    found.proposal.status = "issued";
    found.proposal.reference = `PROP-${new Date().toISOString().slice(0, 7).replace("-", "")}-${String(caseStore.size + 1).padStart(3, "0")}`;
    found.proposal.issuedAt = now(); found.status = "quotation_issued"; found.saleAmountSen = found.proposal.saleAmountSen;
    found.quote = { saleAmountSen: found.proposal.saleAmountSen, averageMonthlyKwh: found.proposal.avgKwh, averageTnbRate: found.proposal.avgRate, quotedSavingsKwh: found.proposal.savingKwhMonth, quotedMonthlySavingsSen: found.proposal.savingRmMonthSen };
    found.financialDocuments = [...(found.financialDocuments ?? []), { id: `document-${caseId}-quotation-${found.proposal.version}`, documentNumber: found.proposal.reference, type: "quotation", amountSen: found.proposal.saleAmountSen, issueDate: input.proposalDate, status: "issued", createdAt: now() }];
    recordActivity(found, actor, "proposal_issued", `Proposal ${found.proposal.reference} issued.`);
    return { ok: true, data: found };
  },
  async regenerateProposal(actor, caseId, input) {
    const found = caseStore.get(caseId); if (!found) return failure("NOT_FOUND", "Case not found.");
    if (found.status !== "quotation_issued" || found.proposal?.status !== "issued") return failure("VALIDATION_ERROR", "Only an unaccepted issued proposal can be regenerated.");
    const voided = await this.voidProposal(actor, caseId, found.proposal.id, "Case details changed; proposal regenerated.");
    if (!voided.ok) return voided;
    return this.issueProposal(actor, caseId, input);
  },
