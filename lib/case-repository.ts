...
export const mockCasesRepository: CasesRepository = {
  ...
  async verifyPayment(actor, input) {
    ...
    if (allocatedKind === "deposit" && ["deposit_pending_verification", "awaiting_deposit_submission"].includes(owner.status)) {
      ...
      owner.status = owner.paymentSchedules?.some((schedule) => schedule.kind === "post_installation" && schedule.status !== "paid") ? hasPending ? "post_installation_payment_pending_verification" : "awaiting_post_installation_payment" : "installed_monitoring";
    }
    recordActivity(owner, actor, "payment_verified", "Payment verified.");
    if (allocatedKind === "post_installation" && owner.status === "installed_monitoring" && !owner.commissionIds?.length) {
