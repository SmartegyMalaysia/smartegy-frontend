...
export type DocumentType = "electricity_bill" | "supporting_document" | "payment_proof" | "signed_proposal" | "quotation" | "invoice" | "receipt" | "other";
...
export interface CreateStaffInput { displayName: string; email: string; phone: string; }
...
export interface CustomerRecord { id: ID; displayName: string; companyRegistrationNumber: string | null; contactName: string | null; email: string | null; phone: string | null; businessType: string | null; businessTypeOther: string | null; }
...
export interface ProposalRecord { id: ID; caseId: ID; version: number; reference: string | null; status: "draft" | "issued" | "accepted" | "void"; proposalDate: ISODate; salesRepName: string; installationAddress: string; installationCostSen: MoneySen; outstationCostSen: MoneySen; saleAmountSen: MoneySen; calculatedDownpaymentSen: MoneySen; deposit1Sen: MoneySen; deposit2Sen: MoneySen; downpaymentTotalSen: MoneySen; balanceSen: MoneySen; financingInterestSen: MoneySen; option1TotalSen: MoneySen; option2TotalSen: MoneySen; option1MonthlySen: MoneySen; option2MonthlySen: MoneySen; avgRate: number; avgKwh: number; avgBillSen: MoneySen; avgDayKwh: number; beforeInstallKwh: number; afterInstallKwh: number; savingKwhMonth: number; savingRmMonthSen: MoneySen; savingRmYearSen: MoneySen; savingRm2YSen: MoneySen; savingRm15YSen: MoneySen; acceptedByName: string | null; acceptanceDate: ISODate | null; selectedTermMonths: 10 | 20 | null; signedDocumentId: ID | null; issuedAt: ISODateTime | null; acceptedAt: ISODateTime | null; }
export interface ProposalInput { salesRepName: string; proposalDate: ISODate; installationAddress: string; installationCostSen: MoneySen; outstationCostSen: MoneySen; saleAmountSen: MoneySen; downpaymentSen?: MoneySen | null; readings: ProposalEnergyReading[]; }
export interface SavingsVerificationInput { readings: ProposalEnergyReading[]; }
export interface VerifiedSavings { readings: ProposalEnergyReading[]; avgKwh: number | null; avgBillSen: MoneySen | null; avgRate: number | null; savingsKwh: number | null; monthlySavingsSen: MoneySen | null; verifiedAt: ISODateTime | null; }
export interface AcceptanceInput { proposalId: ID; acceptedByName: string; acceptanceDate: ISODate; depositDue: ISODate; postInstallationDue: ISODate; selectedTermMonths: 10 | 20; signedProposal: File; }
export interface GeneratedDocumentResult { id: ID; documentNumber: string; type: "quotation" | "proforma"; status: string; signedUrl: string | null; issueDate: ISODate; }
export interface CaseDetail extends CaseSummary { customer: CustomerRecord; service: ServiceRecord; documents: CaseDocument[]; activity: CaseActivity[]; proposal?: ProposalRecord | null; proposalReadings?: ProposalEnergyReading[]; quote?: { saleAmountSen: MoneySen | null; averageMonthlyKwh: number | null; averageTnbRate: number | null; quotedSavingsKwh: number | null; quotedMonthlySavingsSen: MoneySen | null; } | null; verifiedSavings?: VerifiedSavings | null; installationDate?: ISODate | null; installationTime?: string | null; installationProposedDate?: ISODate | null; installationProposedTime?: string | null; installationProposedAt?: ISODateTime | null; installationConfirmationRequestedReason?: string | null; installationConfirmedAt?: ISODateTime | null; monitoringStartedOn?: ISODate | null; trialDecisionOn?: ISODate | null; customerContinues?: boolean | null; installmentTermMonths?: 10 | 20 | null; paymentSchedules?: PaymentSchedule[]; payments?: CasePayment[]; financialDocuments?: Array<{ id: ID; caseDocumentId?: ID; pdfCaseDocumentId?: ID; sourceId?: ID; documentNumber: string; type: "quotation" | "invoice" | "receipt"; amountSen: MoneySen; issueDate: ISODate; status: string; createdAt: ISODateTime; issuedAt?: ISODateTime | null; }>; commissionIds?: ID[]; }
export interface UpdateCaseInput { customer?: Partial<Pick<CustomerRecord, "displayName" | "companyRegistrationNumber" | "contactName" | "email" | "phone" | "businessType" | "businessTypeOther">>; service?: Partial<Pick<ServiceRecord, "siteAddress" | "addressLine1" | "addressLine2" | "postcode" | "city" | "state" | "notes">>; quote?: Partial<{ saleAmountSen: MoneySen; averageMonthlyKwh: number; averageTnbRate: number; quotedSavingsKwh: number; quotedMonthlySavingsSen: MoneySen }>; }
export interface GeneratePaymentScheduleInput { depositDue: ISODate; postInstallationDue: ISODate; }
export interface RecordPaymentInput { amountSen: MoneySen; paymentDate: ISODate; reference?: string | null; proof?: { file: File; fileName: string; mimeType: string; sizeBytes: number } | null; }
...
export interface CaseDocumentInput { type: "electricity_bill" | "supporting_document" | "payment_proof"; fileName: string; mimeType: string; sizeBytes: number; file?: File; }
export interface CreateCaseInput { customer: Pick<CustomerRecord, "displayName" | "contactName" | "email" | "phone" | "businessType" | "businessTypeOther">; service: Pick<ServiceRecord, "siteAddress" | "addressLine1" | "addressLine2" | "postcode" | "city" | "state" | "notes">; documents: CaseDocumentInput[]; }
...
export interface AgentCommissionRecord extends CommissionSummary { customerDisplayName: string; paymentKind?: CommissionPaymentKind; paymentKinds?: CommissionPaymentKind[]; eligibilityStatus: "eligible" | "pending"; lastUpdatedAt: ISODateTime; schedule: CommissionScheduleEntry[]; withheldReason: string | null; adjustmentNote: string | null; qualifyingPaymentDate: ISODate | null; }
...
export interface AgentBankDetails { bankName: string; accountHolderName: string; accountNumber: string; }
...
export interface PayoutTransaction { id: ID; payoutMonth: string; dueDate?: ISODate; agentId: ID; agentName: string; agentCode: string; bankAccount: PayoutBankAccount; commissionId: ID; caseNumber: string; customerDisplayName: string; amountSen: MoneySen; commissionStatus: CommissionStatus; settlementStatus: PayoutSettlementStatus; settledAt: ISODateTime | null; settledById: ID | null; settledByDisplayName: string | null; bankReference: string | null; }
