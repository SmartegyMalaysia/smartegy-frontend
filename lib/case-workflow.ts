...
export type CaseActionKind = "resubmit" | "pass_review" | "request_changes" | "cancel" | "delete_case" | "generate_schedule" | "accept_proposal" | "verify_payment" | "record_installation" | "verify_savings" | "move_to_trial" | "accept_trial";
...
export function caseActionLabels(status: CaseStatus, role: CurrentUser["role"], hasPaymentSchedule = false, depositPaid = false): CaseAction[] {
  ...
  switch (status) {
    case "under_review": return [{ kind: "pass_review", label: "Prepare Proposal", variant: "primary" }, { kind: "request_changes", label: "Request Changes", variant: "secondary", requiresReason: true }, { kind: "cancel", label: "Cancel Case", variant: "danger", requiresReason: true }];
    case "quotation_issued": return [...(hasPaymentSchedule ? [] : [{ kind: "accept_proposal" as const, label: "Accept Proposal", variant: "primary" as const }]), { kind: "cancel", label: "Cancel Case", variant: "danger", requiresReason: true }];
