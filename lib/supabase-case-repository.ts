...
import type { AcceptTrialInput, AcceptanceInput, CaseDetail, CaseDocument, CaseStatus, CurrentUser, CreateCaseInput, GeneratedDocumentResult, GeneratePaymentScheduleInput, ID, ProposalEnergyReading, ProposalInput, RecordPaymentInput, UpdateCaseInput, VerifyPaymentInput } from "./types";
...
async function loadCase(caseId: string): Promise<CaseDetail> {
  ...
  const { data: savingsReadings, error: savingsReadingsError } = await supabase.rpc("get_case_savings_readings", { p_case_id: caseId });
  if (savingsReadingsError) throw savingsReadingsError;
  ...
  const proposalDocumentFields = proposalRow?.calculation_snapshot && typeof proposalRow.calculation_snapshot === "object" ? proposalRow.calculation_snapshot : {};
  return {
    ...
    customer: { id: baseCase.customer_id, displayName: row.customer_name, companyRegistrationNumber: row.registration_number, contactName: row.contact_name, email: customer?.email ?? null, phone: customer?.phone ?? null, businessType: customer?.business_type ?? null, businessTypeOther: customer?.business_type_other ?? null },
    ...
    verifiedSavings: (savingsReadings?.length || baseCase.savings_verified_at) ? (() => {
      const readings: ProposalEnergyReading[] = (savingsReadings ?? []).map((reading: any) => ({ sequence: Number(reading.sequence_no), month: reading.month_label, tnbRate: Number(reading.tnb_rate), kwhUsed: Number(reading.kwh_used), billAmountSen: moneyToSen(reading.bill_amount) ?? 0, operationDays: Number(reading.operation_days), dailyKwh: Number(reading.daily_kwh) }));
      return { readings, avgKwh: readings.length ? readings.reduce((sum, reading) => sum + reading.kwhUsed, 0) / readings.length : null, avgBillSen: readings.length ? Math.round(readings.reduce((sum, reading) => sum + reading.billAmountSen, 0) / readings.length) : null, avgRate: readings.length ? readings.reduce((sum, reading) => sum + reading.tnbRate, 0) / readings.length : null, savingsKwh: baseCase.verified_savings_kwh == null ? null : Number(baseCase.verified_savings_kwh), monthlySavingsSen: moneyToSen(baseCase.verified_monthly_savings_rm), verifiedAt: baseCase.savings_verified_at ?? null };
    })() : null,
...
export const supabaseCasesRepository: CasesRepository = {
  ...
  async acceptProposal(_actor, caseId, input: AcceptanceInput) {
    ...
    try {
      ...
      if (latestCase.proposal?.status === "accepted") return { ok: true, data: latestCase };
    } catch (error) {
